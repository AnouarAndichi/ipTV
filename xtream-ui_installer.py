...
rVersions = {"16.04": "xenial", "18.04": "bionic"}
...
def installadminpanel():
    ...
    os.system(
        "unzip -o /tmp/update.zip -d /tmp/update/ > /dev/null && cp -rf /tmp/update/XtreamUI-master/* /home/xtreamcodes/iptv_xtream_codes/ > /dev/null && rm -rf /tmp/update/XtreamUI-master > /dev/null && rm -rf /tmp/update > /dev/null && chown -R xtreamcodes:xtreamcodes /home/xtreamcodes > /dev/null"
    )
    try:
        os.remove("/tmp/update.zip")
    except:
    ...
    if os.path.exists("/tmp/update2.zip"):
        ...
        os.system(
            "unzip -o /tmp/update2.zip -d /tmp/update2/ > /dev/null && cp -rf /tmp/update2/* /home/xtreamcodes/iptv_xtream_codes/ > /dev/null && rm -rf /tmp/update2/* > /dev/null && rm -rf /tmp/update2 > /dev/null && chown -R xtreamcodes:xtreamcodes /home/xtreamcodes/ > /dev/null > /dev/null"
        )
        return True
    printc("Failed to download installation file!", col.FAIL)
    return False
...
def mysql(rUsername, rPassword):
    ...
    for i in range(5):
        ...
        try:
            if rDrop:
                os.system(
                    "mysql -u root%s -e \"DROP USER IF EXISTS '%s'@'%%'; DROP USER IF EXISTS '%s'@'localhost'; DROP USER IF EXISTS '%s'@'127.0.0.1';\" > /dev/null"
                    % (rExtra, rUsername, rUsername, rUsername)
                )
                os.system(
                    'mysql -u root%s -e "DROP DATABASE IF EXISTS xtream_iptvpro; CREATE DATABASE IF NOT EXISTS xtream_iptvpro;" > /dev/null'
                    % rExtra
                )
                os.system(
                ...
                os.system(
                    "mysql -u root%s -e \"USE xtream_iptvpro; UPDATE settings SET live_streaming_pass = '%s', unique_id = '%s', crypt_load_balancing = '%s', get_real_ip_client='';\" > /dev/null"
                    % (rExtra, generate(20), generate(12), generate(20))
                )
                os.system(
                    "mysql -u root%s -e \"USE xtream_iptvpro; REPLACE INTO streaming_servers (id, server_name, domain_name, server_ip, vpn_ip, ssh_password, ssh_port, diff_time_main, http_broadcast_port, total_clients, system_os, network_interface, latency, status, enable_geoip, geoip_countries, last_check_ago, can_delete, server_hardware, total_services, persistent_connections, rtmp_port, geoip_type, isp_names, isp_type, enable_isp, boost_fpm, http_ports_add, network_guaranteed_speed, https_broadcast_port, https_ports_add, whitelist_ips, watchdog_data, timeshift_only) VALUES (1, 'Main Server', '', '%s', '', NULL, NULL, 0, 8080, 1000, '%s', 'eth0', 0, 1, 0, '', 0, 0, '{}', 3, 0, 8880, 'low_priority', '', 'low_priority', 0, 1, '', 1000, 8443, '', '[\"127.0.0.1\",\"\"]', '{}', 0);\" > /dev/null"
                    % (rExtra, getIP(), getVersion())
                )
                os.system(
                    "mysql -u root%s -e \"USE xtream_iptvpro; REPLACE INTO reg_users (id, username, password, email, member_group_id, verified, status) VALUES (1, 'admin', '\$6\$rounds=20000\$xtreamcodes\$XThC5OwfuS0YwS4ahiifzF14vkGbGsFF1w7ETL4sRRC5sOrAWCjWvQJDromZUQoQuwbAXAFdX3h3Cp3vqulpS0', 'admin@website.com', 1, 1, 1);\" > /dev/null"
                    % rExtra
                )
                os.system(
                    "mysql -u root%s -e \"CREATE USER '%s'@'localhost' IDENTIFIED BY '%s'; GRANT ALL PRIVILEGES ON xtream_iptvpro.* TO '%s'@'localhost' WITH GRANT OPTION; GRANT SELECT, PROCESS, LOCK TABLES ON *.* TO '%s'@'localhost';FLUSH PRIVILEGES;\" > /dev/null"
                    % (rExtra, rUsername, rPassword, rUsername, rUsername)
                )
                os.system(
                    "mysql -u root%s -e \"CREATE USER '%s'@'127.0.0.1' IDENTIFIED BY '%s'; GRANT ALL PRIVILEGES ON xtream_iptvpro.* TO '%s'@'127.0.0.1' WITH GRANT OPTION; GRANT SELECT, PROCESS, LOCK TABLES ON *.* TO '%s'@'127.0.0.1';FLUSH PRIVILEGES;\" > /dev/null"
                    % (rExtra, rUsername, rPassword, rUsername, rUsername)
                )
                os.system(
                    "mysql -u root%s -e \"GRANT SELECT, INSERT, UPDATE, DELETE ON xtream_iptvpro.* TO '%s'@'%%' IDENTIFIED BY '%s';FLUSH PRIVILEGES;\" > /dev/null"
                ...
                os.system(
                    "mysql -u root%s -e \"USE xtream_iptvpro; UPDATE settings SET get_real_ip_client='', double_auth='1', hash_lb='1', mag_security='1' where id='1';\" > /dev/null"
                    % rExtra
                )
                if not os.path.exists("/etc/mysql/mysqld"):
                    if (
                        not "EnvironmentFile=-/etc/mysql/mysqld"
                        in open("/lib/systemd/system/mysql.service").read()
                    ):
                        os.system(
                            'echo "LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libjemalloc.so.1" > /etc/mysql/mysqld'
                        )
                        os.system(
                            'echo "%s" > /lib/systemd/system/mysql.service'
                            % rMySQLServiceFile
                        )
                        os.system(
...
def encrypt(
    ...
    rf.write(
        "".join(
            chr(ord(c) ^ ord(k))
            for c, k in izip(
                '{"host":"%s","db_user":"%s","db_pass":"%s","db_name":"%s","server_id":"%d", "db_port":"%d", "pconnect":"0"}'
...
def configure():
    ...
    with open(
        "/home/xtreamcodes/iptv_xtream_codes/nginx/conf/nginx.conf", "r"
    ) as nginx_file:
    ...
    with open(
        "/home/xtreamcodes/iptv_xtream_codes/nginx/conf/nginx.conf", "w"
    ) as nginx_file:
        nginx_file.write(nginx_replace)
    os.system(
        "wget -q https://bitbucket.org/emre1393/xtreamui_mirror/downloads/nginx -O /home/xtreamcodes/iptv_xtream_codes/nginx/sbin/nginx"
    )
    os.system(
        "wget -q https://bitbucket.org/emre1393/xtreamui_mirror/downloads/nginx_rtmp -O /home/xtreamcodes/iptv_xtream_codes/nginx_rtmp/sbin/nginx_rtmp"
    )
    os.system(
        "wget -q https://bitbucket.org/emre1393/xtreamui_mirror/downloads/pid_monitor.php -O /home/xtreamcodes/iptv_xtream_codes/crons/pid_monitor.php"
    )
    ...
    os.system(
        "wget -q https://bitbucket.org/emre1393/xtreamui_mirror/downloads/GeoLite2.mmdb -O /home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb"
    )
    os.system(
        "chattr -f +i /home/xtreamcodes/iptv_xtream_codes/GeoLite2.mmdb > /dev/null"
    )
    ...
    if (
        not "@reboot root /home/xtreamcodes/iptv_xtream_codes/start_services.sh"
        in open("/etc/crontab").read()
    ):
        os.system(
...
if __name__ == "__main__":
    ...
    print(
        "%s │ Check out the mirror repo: https://bitbucket.org/emre1393/xtreamui_mirror %s"
        ...
        if len(rHost) > 0 and len(rPassword) > 0 and rServerID > -1:
            ...
            if raw_input("  ").upper() == "Y":
                ...
                if rType.upper() == "MAIN":
                    installadminpanel()
                    os.system(
                        "sed -i 's|randomcodehere|%s|g' /home/xtreamcodes/iptv_xtream_codes/nginx/conf/admin_panel.conf"
                        % rAccesscode
                    )
                configure()
                start()
